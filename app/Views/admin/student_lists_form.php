<dialog id="addStudentModal" class="rounded-lg shadow-lg p-0 w-full max-w-md mx-auto my-auto text-green-900">
  ...
  echo form_open('admin/add_student', ['id' => 'addStudentForm', 'class' => 'grid grid-cols-1 md:grid-cols-2 gap-2 space-y-4 p-6']);

  echo form_label('Add New Student', 'title', ['class' => 'md:col-span-2 text-2xl font-bold text-center mb-4 border-b-2 border-green-500 w-full']);
  echo form_input([
    'name' => 'title',
    'id' => 'title',
    'class' => 'hidden',
  ...
  echo '<div>';
  echo form_label('Student No.', 'student_no', ['class' => 'block text-sm font-medium']);
  ...
  echo '<div>';
  echo form_label('Sex', 'sex', ['class' => 'block text-sm font-medium']);
  echo form_dropdown('sex', [
    '' => 'Select Sex',
    'Male' => 'Male',
    'Female' => 'Female'
  ], '', [
  ...
  echo '<div>';
  echo form_label('Course', 'course', ['class' => 'block text-sm font-medium']);
  ...
  echo '<div>';
  echo form_label('Year Level', 'year_level', ['class' => 'block text-sm font-medium']);
  echo form_input('year_level', '', [
    'id' => 'year_level',
    'class' => 'mt-1 block w-full border border-gray-300 rounded px-3 py-2 hover:scale-105',
    'required' => 'required',
    'type' => 'number',
  ...
  echo form_submit('submit', 'Add Student', ['class' => 'w-full bg-green-600 text-white px-4 py-2 rounded']);
  echo '</div>';
  echo form_button([
    'type' => 'button',
